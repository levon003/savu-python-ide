...
class AboutDialog extends org.fife.ui.AboutDialog {
	 ...
	 */
	public AboutDialog(final Savu rtext) {
		...
		JTable pluginTable = new JTable(createTableData(rtext),
			 ...
			 * method JTable#setFillsViewportHeight(boolean). 1.5: Change me to
			 * method call instead.
			 */
	...
	protected JPanel createAboutApplicationPanel() {
		...
		JPanel panel = UIUtil.newTabbedPanePanel();
		Savu rtext = (Savu) getOwner();
		InfoPane editor = new InfoPane(rtext);
		// W3C_LENGTH_UNITS ="JEditorPane.w3cLengthUnits" in 1.5, but we're
		// 1.4-compatible.
	...
	private static final JPanel createLibrariesPanel(ResourceBundle msg) {
		...
		appendLibrary(sb, "Groovy:", "http://groovy.codehaus.org/",
	 ...
	 * 
	 * @param app
	 *            The GUI Application containing plugins.
	...
	private static class InfoPane extends SelectableLabel implements
			HyperlinkListener {

		private Savu rtext;

		public InfoPane(Savu rtext) {
			this.rtext = rtext;
		...
		public String getContentText() {
			String version = rtext == null ? "firstTime" : rtext
					.getVersionString();
			String text = "<html><body><center>"
					+ "Version "
					+ version
					+ "<br>Savu - A Comprehensive Project for the Carleton College CS Department"
					+ "<br>"
					+ "RText - Copyright (c) 2013 Robert Futrell<br>"
