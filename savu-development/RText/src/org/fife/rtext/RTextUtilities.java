...
public class RTextUtilities {
	...
	/**
	 * The last LookAndFeel (attempted to be) installed. This may not be the
	 * actual currently-active LookAndFeel, if the user tries to install one
	 * with different window decoration properties.
	 */
	 ...
	 *            The resource bundle.
	 * @param key
	 *            The key to use in <code>msg</code> for the file filter's
	 *            description.
	 * @param extensions
	 *            Either a string representing a single extension or an array of
	 *            strings containing multiple extensions.
	 */
	 ...
	 * 
	 * @param fnfd
	 *            The <code>FindInFilesDialog</code> to configure.
	 */
	 ...
	 *            The component.
	 * @param iconPanel
	 *            The icon panel. If this is <code>null</code>, then a spacer is
	 *            used.
	 * @return The panel.
	 */
	 ...
	 * 
	 * @param rtext
	 *            The RText instance that will own this file chooser.
	 * @return A file chooser for RText to use.
	 * @see #saveFileChooserFavorites(Savu)
	 */
	public static final RTextFileChooser createFileChooser(Savu rtext) {
		...
		try {
			...
			ResourceBundle msg = ResourceBundle
					.getBundle("org.fife.rtext.FileFilters");

			// Add (localized) file filters.
			addFilter(chooser, msg, "ActionScript",
					new String[] { "as", "asc" });
			addFilter(chooser, msg, "AssemblerX86", "asm");
			addFilter(chooser, msg, "BBCode", "bbc");
			addFilter(chooser, msg, "CPlusPlus", new String[] { "c", "cpp",
					"cxx", "h" });
			...
			addFilter(chooser, msg, "Fortran", new String[] { "f", "for",
					"fort", "f77", "f90" });
			addFilter(chooser, msg, "Groovy", new String[] { "groovy", "grv" });
			addFilter(chooser, msg, "HTML", new String[] { "htm", "html" });
			...
			addFilter(chooser, msg, "LaTeX", new String[] { "tex", "ltx",
					"latex" });
			addFilter(chooser, msg, "Lisp", new String[] { "cl", "clisp", "el",
					"l", "lisp", "lsp", "ml" });
			addFilter(chooser, msg, "Lua", "lua");
			addFilter(chooser, msg, "Makefile", new String[] { "Makefile",
					"makefile" });
			addFilter(chooser, msg, "Nsis", "nsi");
			addFilter(chooser, msg, "Perl", new String[] { "pl", "perl", "pm" });
			addFilter(chooser, msg, "PHP", new String[] { "php" });
			...
			addFilter(chooser, msg, "UnixShell", new String[] { "sh", "bsh",
					"csh", "ksh" });
			addFilter(chooser, msg, "VisualBasic", "vb");
			addFilter(chooser, msg, "WindowsBatch",
					new String[] { "bat", "cmd" });
			addFilter(chooser, msg, "XML", new String[] { "xml", "xsl", "xsd",
					"wsdl", "jnlp", "macro", "manifest" });
			...
			chooser.addChoosableFileFilter(new ExtensionFileFilter(msg
					.getString("AllSupported"), new String[] { "as", "asc",
					"asm", "bbc", "c", "cpp", "cxx", "h", "clj", "cs", "css",
					"pas", "dtd", "mxml", "f", "for", "fort", "f77", "f90",
					"groovy", "grv", "htm", "html", "java", "js", "json",
					"jsp", "tex", "ltx", "latex", "cl", "clisp", "el", "l",
					"lisp", "lsp", "ml", "lua", "Makefile", "makefile", "nsi",
					"perl", "pl", "pm", "php", "properties", "py", "rb", "sas",
					"scala", "sql", "tcl", "txt", "sh", "bsh", "csh", "ksh",
					"vb", "bat", "cmd", "xml", "xsl", "xsd", "wsdl", "jnlp",
					"macro", "manifest", },
					ExtensionFileFilter.SYSTEM_CASE_CHECK, false));
	 ...
	 * 
	 * @param enabled
	 *            Whether templates should be enabled.
	 * @return <code>true</code> if everything went okay; <code>false</code> if
	 *         the method failed.
	 */
	public static boolean enableTemplates(Savu rtext, boolean enabled) {
		...
		if (old != enabled) {
			...
			if (enabled) {
				...
				return RSyntaxTextArea
						.setTemplateDirectory(f.getAbsolutePath());
			}
		}
		return true;
	}
	 ...
	 *            <code>null</code>, they are simply removed.
	 * @param inPreBlock
	 *            Whether this HTML will be in within <code>pre</code> tags. If
	 *            this is <code>true</code>, spaces will be kept as-is;
	...
	/**
	 * Returns the file in which to save file chooser favorite directories. This
	 * file should have the encoding UTF-8.
	 * 
	 * @return The file.
	 */
	 ...
	 * 
	 * @param fileName
	 *            The file from which to get the image (must be .jpg, .gif or
	 *            .png).
	 ...
	 * starts up. This may not be the same thing as the currently active LAF, if
	 * the user chose a LAF that used custom window decorations, for example.
	 * 
	 ...
	 * 
	 * @param macroFile
	 *            A file containing an <code>RTextArea</code> macro. If this
	 *            file is <code>null</code>, then <code>null</code> is returned.
	...
	/**
	 * Returns the directory in which to load and save user preferences (beyond
	 * those saved via the Java preferences API).
	 * 
	 * @return The directory.
	 */
	public static File getPreferencesDirectory() {
		return new File(System.getProperty("user.home"), ".savu");
	}

	/**
	 * Converts a <code>String</code> representing a wildcard file filter into a
	 * <code>Pattern</code> containing a regular expression good for finding
	 * files that match the wildcard expression.
	 * <p>
	 * 
	 * Example: For
	 * <p>
	 ...
	 * 
	 * @param fileFilter
	 *            The file filter for which to create equivalent regular
	 *            expressions. This filter can currently only contain the
	 *            wildcards '*' and '?'.
	 * @param showErrorDialog
	 *            If <code>true</code>, an error dialog is displayed if an error
	 *            occurs.
	 ...
	 */
	/*
	 * FIXME: Have me return the file list in alphabetical order (as this is not
	 * guaranteed by File.listFiles()).
	 */
	 ...
	 * 
	 * @param image
	 *            The <code>java.awt.Image</code> on which to apply the alpha
	 *            filter.
	 * @param alpha
	 *            The alpha value to use when defining how translucent you want
	 *            the image to be. This should be in the range 0.0f to 1.0f.
	 */
	public static BufferedImage getTranslucentImage(Savu rtext, Image image,
			float alpha) {
	 ...
	 * 
	 * @param array
	 *            The array. If this is <code>null</code> or has zero length,
	 *            <code>null</code> is returned.
	 ...
	 *            <code>null</code> is returned.
	 * @param connector
	 *            Text to connect each array element with.
	 * @return The joined text.
	 * @see #join(String[])
	 */
	 ...
	 * 
	 * @param rtext
	 *            The RText instance in which to open the files.
	 * @param directory
	 *            The top of the directory tree, all files in which you want
	 *            opened in RText.
	 */
	public static void openAllFilesIn(Savu rtext, File directory) {
	...
	/**
	 * Remove problematic actions that prevent Ctrl+PageUp/PageDown from being
	 * used for cycling through active documents.
	 * 
	 * @param c
	...
	/**
	 * Saves the "Favorite Directories" of RText's file chooser. It is assumed
	 * that the file chooser has been created via
	 * {@link #createFileChooser(Savu)} before calling this method.
	 * 
	 * If an error occurs saving the favorites, an error message is displayed.
	 * 
	 * @param rtext
	 *            The parent RText instance.
	 * @see #createFileChooser(Savu)
	 */
	public static void saveFileChooserFavorites(Savu rtext) {
	...
	/**
	 * Toggles whether the experimental "drop shadows" option is enabled. Note
	 * that this may do nothing if we're running on Java 1.4 or 1.5, or the
	 * current system does not support translucent windows.
	 * 
	 * @param enabled
						...
						PopupWindowDecorator.set(new PopupWindowDecorator() {
							public void decorate(JWindow window) {
								...
								if (cp instanceof JComponent) {
									TranslucencyUtil util = TranslucencyUtil
											.get();
									util.setOpaque(window, false);
									((JComponent) cp).setBorder(BorderFactory
	 ...
	 * 
	 * @param rtext
	 *            An RText instance to display a message if an exception is
	 *            thrown.
	 * @param lnfClassName
	 *            The class name of the Look and Feel to set.
	 */
	// TODO: Shouldn't this be in UIUtil somehow, for other GUIApplications?
	public static void setLookAndFeel(final Savu rtext, String lnfClassName) {
			...
			try {
				...
				if (curSubstance != nextSubstance) {
					String message = rtext
							.getString("Info.LookAndFeel.LoadOnNextRestart");
