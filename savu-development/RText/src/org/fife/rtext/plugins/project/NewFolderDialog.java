...
public class NewFolderDialog extends EscapableDialog {
	 ...
	 * 
	 * @param parent
	 *            The window that owns this directory chooser.
	 */
	public NewFolderDialog(Savu parent) {
		this(parent, null);
	}
	 ...
	 *            The window that owns this directory chooser.
	 * @param entry
	 *            If non-<code>null</code>, this will be a dialog to edit the
	 *            entry, instead of create a new one.
	 */
	public NewFolderDialog(Savu parent, FolderProjectEntry entry) {
		super(parent);
		init(parent);
		if (entry != null) {
	 ...
	 * 
	 * @return The filtering information selected, or <code>null</code> if the
	 *         directory was cancelled.
	 */
	...
	private void init(Window parent) {
		...
		scrollPane
				.setHorizontalScrollBarPolicy(RScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);
		scrollPane
				.setVerticalScrollBarPolicy(RScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
		...
		JLabel inFilterLabel = new JLabel(
				Messages.getString("FolderDialog.InFilter"));
		inFilterLabel.setDisplayedMnemonic(Messages
				.getMnemonic("FolderDialog.InFilter.Mnemonic"));
		...
		JLabel outFilterLabel = new JLabel(
				Messages.getString("FolderDialog.OutFilter"));
		outFilterLabel.setDisplayedMnemonic(Messages
				.getMnemonic("FolderDialog.OutFilter.Mnemonic"));
		...
		JLabel outFolderLabel = new JLabel(
				Messages.getString("FolderDialog.OutFolders"));
		outFolderLabel.setDisplayedMnemonic(Messages
				.getMnemonic("FolderDialog.OutFolders.Mnemonic"));
		...
		container.add(cp);
		descLabel = new SelectableLabel(
				Messages.getString("FolderDialog.FolderSelect.Desc"));
	...
	private void moveForwardOneStep() {
		...
		if (panelIndex == PANELS.length - 1) {
			...
			if (panelIndex == PANELS.length - 1) {
				...
				okButton.setMnemonic(Messages
						.getMnemonic("Button.Finish.Mnemonic"));
				descLabel.setText(Messages
						.getString("FolderDialog.Filters.Desc"));
			}
		}
	}
	...
	/**
	 * Selects the specified directory, if it exists. Otherwise, the selection
	 * is cleared.
	 * 
	 * @param dir
	 *            The directory to select.
	...
	/**
	 * Configures this dialog to edit an existing folder project entry, instead
	 * of creating a new one.
	 * 
	 * @param entry
	 *            The entry to initialize all fields with.
	 */
	...
	private class Listener implements ActionListener, TreeSelectionListener,
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (okButton == source) {
				...
				okButton.setMnemonic(Messages
						.getMnemonic("Button.Next.Mnemonic"));
				descLabel.setText(Messages
						.getString("FolderDialog.FolderSelect.Desc"));
		...
		public void propertyChange(PropertyChangeEvent e) {
			...
			if (property.equals(DirectoryTree.WILL_EXPAND_PROPERTY)) {
				NewFolderDialog.this.setCursor(Cursor
