...
public class SpellingSupport implements SpellingParserListener {
	 ...
	 */
	public static final String[] DICTIONARIES = { "BritishEnglish",
			"AmericanEnglish", };
	 ...
	 */
	public SpellingSupport(Savu rtext) {
		this.rtext = rtext;
	}
	...
	/**
	 * Creates a spelling parser using the current spelling preferences set in
	 * this view, and assigns {@link #spellingParser} to it.
	 * 
	 * @throws IOException
	...
	private void createSpellingParser() throws IOException {
		...
		spellingParser = SpellingParser.createEnglishSpellingParser(file,
				american);
		...
		try {
			...
			String desc = rtext.getString(
					...
					userDictionary == null ? "null" : userDictionary
	...
	/**
	 * Returns the spelling parser. Note that this may be <code>null</code> if
	 * spell checking has not yet been enabled.
	 * 
	 ...
	 * 
	 * @param dict
	 *            The dictionary. If this is unknown, American English is used.
	...
	public void setSpellingDictionary(String dict) {
		...
		if (dict != null && !dict.equals(spellingDictionary)) {
			...
			if (view != null && spellingParser != null
			...
			if (spellingParser != null) { // spell checking may be disabled
				try {
					...
					String desc = rtext
							.getString("Error.LoadingSpellingParser.txt");
					rtext.displayException(ioe, desc);
					return;
				}
			}
	 ...
	 * 
	 * @param dict
	 *            The new user dictionary. If this is <code>null</code>, then
	...
	/**
	 * Called when the user adds a word to their user dictionary, or chooses to
	 * ignore a word.
	 * 
	 * @param e
	 *            The event.
	...
	/**
	 * Toggles whether the spelling parser is installed on all currently visible
	 * text areas. This should only be called on the EDT.
	 */
	private void toggleSpellingParserInstalled() {
		...
		if (spellingParser != null) { // Should always be true.
			...
			Exception e = new Exception(
					"Internal error: Spelling parser is null!");
	...
	private class ViewSpellingErrorWindowAction extends StandardAction {
		...
		public ViewSpellingErrorWindowAction(Savu app) {
			super(app, app.getResourceBundle(), "SpellingErrorList.MenuItem");
		}

		public void actionPerformed(ActionEvent e) {
			//TODO LOW PRIO remove this
		}
