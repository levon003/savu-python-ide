...
public abstract class AbstractWorkspaceTreeNode extends DefaultMutableTreeNode {
	 ...
	 * returns <code>false</code>, since for most entities this operation does
	 * not make sense.
	 * 
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (rc == RTextFileChooser.APPROVE_OPTION) {
				...
				for (int i = 0; i < toAdd.length; i++) {
					...
					FileProjectEntryTreeNode childNode = new FileProjectEntryTreeNode(
							plugin, entry);
	...
	protected class AddFolderAction extends BaseAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (dir != null) {
				...
				FolderProjectEntry entry = new FolderProjectEntry(parent,
						dirFile);
				parent.addEntry(entry);
				FolderProjectEntryTreeNode childNode = new FolderProjectEntryTreeNode(
						plugin, entry);
	...
	protected class AddLogicalFolderAction extends BaseAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (name != null) {
				LogicalFolderProjectEntry entry = new LogicalFolderProjectEntry(
						this.parent, name);
				this.parent.addEntry(entry);
				MutableTreeNode child = new LogicalFolderProjectEntryTreeNode(
						plugin, entry);
