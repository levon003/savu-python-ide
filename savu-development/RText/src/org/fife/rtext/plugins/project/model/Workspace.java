...
public class Workspace implements ModelEntity {
	 ...
	 */
	public static Workspace load(ProjectPlugin plugin, File file)
			throws IOException {
			...
			if (childNode.getNodeType() == Node.ELEMENT_NODE) {
				...
				if (ProjectEntry.DIR_PROJECT_ENTRY.equals(tag)) {
					...
					FolderFilterInfo info = new FolderFilterInfo(
							displayedFiles, hiddenFiles, hiddenFolders);
					...
					recursivelyAddChildren(childElem,
							(ProjectEntryParent) entry);
				}
	...
	/**
	 * Gives this workspace a new name. Immediately creates a new XML file in
	 * the same directory containing the previous workspace file.
	 * 
	 * @param name
