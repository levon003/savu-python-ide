 ...
 * @version 1.0
 */
class HeapIndicatorOptionPanel extends PluginOptionsDialogPanel implements
	...
	public HeapIndicatorOptionPanel(AbstractPluggableGUIApplication app,
		...
		topPanel2.setBorder(BorderFactory.createCompoundBorder(
				new OptionPanelBorder(msg
						.getString("Plugin.OptionPanel.Title.General")),
				empty5Border));
		...
		topPanel2.setBorder(BorderFactory.createCompoundBorder(
				new OptionPanelBorder(msg
						.getString("Plugin.OptionPanel.Title.Appearance")),
				empty5Border));

		// Panel for the "use system colors" check box.
		temp = new JPanel(new BorderLayout());
		systemColorsCheckBox = new JCheckBox(
				msg.getString("Plugin.OptionPanel.SystemColors.text"));
		...
		Border indentBorder = orientation.isLeftToRight() ? BorderFactory
				.createEmptyBorder(0, 20, 0, 0) : BorderFactory
		...
		if (orientation.isLeftToRight()) {
			temp.add(foregroundLabel);
			temp.add(foregroundButton);
			temp.add(borderLabel);
			temp.add(borderButton);
		} else {
			temp.add(foregroundButton);
			temp.add(foregroundLabel);
			temp.add(borderButton);
	...
	/**
	 * Updates the heap indicator's parameters to reflect those in this options
	 * panel.
	 */
	...
	protected OptionsPanelCheckResult ensureValidInputsImpl() {
		...
		if (!(number instanceof Integer)) {
			ResourceBundle msg = ((HeapIndicatorPlugin) getPlugin())
					.getBundle();
			String error = msg
					.getString("Plugin.OptionPanel.Error.RefreshRate.text");
	...
	public void propertyChange(PropertyChangeEvent e) {
		...
		if (name.equals(RColorSwatchesButton.COLOR_CHANGED_PROPERTY)) {
			hasUnsavedChanges = true;
			firePropertyChange(COLOR_PROPERTY, e.getOldValue(), e.getNewValue());
		}
	}

	/**
	 * Updates this panel's displayed parameter values to reflect those of the
	 * heap indicator.
	 */
	...
	/**
	 * Enables or disables the widgets concerned with setting colors for this
	 * heap indicator.
	 * 
	 * @param enabled
	 *            Whether or not the widgets should be enabled.
	 */
	...
	public void stateChanged(ChangeEvent e) {
		hasUnsavedChanges = true;
		firePropertyChange(REFRESH_RATE_PROPERTY, null,
				refreshRateSpinner.getValue());
	}
