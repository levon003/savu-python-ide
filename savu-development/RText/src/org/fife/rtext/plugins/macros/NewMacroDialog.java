...
public class NewMacroDialog extends EscapableDialog {
	...
	public void createGUI(MacroPlugin plugin) {
		...
		JPanel namePanel = RTextUtilities.createAssistancePanel(nameField,
				nameDIP);
		JLabel descLabel = UIUtil.newLabel(msg, "Label.Desc");
		descField = new JTextField(40);
		descLabel.setLabelFor(descField);
		JPanel descPanel = RTextUtilities
				.createAssistancePanel(descField, null);
		...
		JPanel shortcutPanel = RTextUtilities.createAssistancePanel(
				shortcutField, null);
		...
		JPanel typePanel = RTextUtilities
				.createAssistancePanel(typeCombo, null);
		...
		cancelButton = UIUtil.newButton(parentMsg, "Cancel", "CancelMnemonic");
	...
	private File createMacroFile() {
		...
		return new File(plugin.getMacroDir(), nameField.getText()
				+ EXTENSIONS[typeCombo.getSelectedIndex()]);
	}
	...
	private void okPressed() {
		...
		if (isNew) {
			...
			if (MacroManager.get().containsMacroNamed(name)) {
				...
				rc = JOptionPane.showConfirmDialog(NewMacroDialog.this, text,
	...
	/**
	 * Initializes the fields in this dialog to show the values for a specific
	 * macro.
	 * 
	 * @param macro
	 *            The macro.
	...
	public void setMacro(Macro macro) {
		...
		editButton = UIUtil.newButton(msg, "Button.Edit",
				"Button.Edit.Mnemonic");
	 ...
	 * 
	 * @param visible
	 *            Whether this dialog is to be made visible.
	 */
	...
	private class Listener implements ActionListener, DocumentListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (source == typeCombo) {
				engineNotesLabel.setText(getEngineNote(typeCombo
						.getSelectedIndex()));
