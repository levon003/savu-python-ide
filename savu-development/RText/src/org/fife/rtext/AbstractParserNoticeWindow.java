...
public abstract class AbstractParserNoticeWindow extends DockableWindow {
	...
	public AbstractParserNoticeWindow(Savu rtext) {
		this.rtext = rtext;
	}
	...
	protected JTable createTable(TableModel model) {
		table = new JTable() {
			 ...
			 * method JTable#setFillsViewportHeight(boolean). 1.6: Remove this
			 * and replace it with the method call.
			 */
	 ...
	 * delegating TableCellRenderer on a JTableHeader can cause NPE's. Then, the
	 * UI never repaints from then on. Seems to happen only when the table is
	 * actually visible during the LaF change. See <a
	 * href="http://bugs.sun.com/view_bug.do?bug_id=6429812">6429812</a>.
	 */
	public void updateUI() {
		 ...
		 * to what it was originally. This prevents the NPE from the JRE bug,
		 * since we're no longer using the renderer with a cached
		 * Windows-specific TableCellRenderer (when Windows LaF is enabled). 2.
		 * Update the UI, like normal. 3. After the update, we must explicitly
		 * re-set the JTableHeader as the column view in the enclosing
		 * JScrollPane. This is done by default the first time you add a JTable
		 * to a JScrollPane, but since we gave the JTable a new header as a
		 * workaround for the JRE bug, we must explicitly tell the JScrollPane
		 * about it as well.
		 */
	...
	protected abstract class ParserNoticeTableModel extends DefaultTableModel {
		...
		public ParserNoticeTableModel(String lastColHeader) {
			String[] colHeaders = new String[] { "",
					...
					rtext.getString("ParserNoticeList.Line"), lastColHeader, };
