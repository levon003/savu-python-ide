...
/**
 * A transfer handler for RText's main view capable of receiving files from
 * drag-and-drop.
 * <p>
 * 
 * Note that this class is only used by the split pane view and MDI view, not by
 * the tabbed pane view. This is because the tabbed pane view has its own
 ...
 * <code>canImport</code> and <code>importData</code> will return these child
 * components and not the main view, so we need to know what it is ahead of
 * time.
 * 
...
class MainPanelTransferHandler extends TransferHandler {
	...
	/**
	 * Says that we cannot move or copy data from this main view, only add to
	 * it.
	 * 
	 * @param c
	 ...
	 * 
	 * @param flavors
	 *            The flavors in which to check for the file flavor.
	 ...
	 * a new tab for the "dragged" file(s) and places it in the destination
	 * <code>AbstractMainView</code>.
	 * 
	 * @param c
	 *            The component receiving the "drop".
	 * @param t
	 *            The data being transfered (information about the file).
	...
	/**
	 * Does the dirty work of importing file data. This method is static so that
	 * it can be called by <code>TabbedPaneViewTransferHandler</code>, which
	 * unfortunately must derive from a different superclass.
	 * 
	 * @param mainView
	 *            The main view receiving the data.
	 * @param c
	 *            The component receiving the "drop".
	 * @param t
	 *            The data being transfered (information about the file).
