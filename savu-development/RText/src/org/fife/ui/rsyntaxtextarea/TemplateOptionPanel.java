 ...
 * <code>org.fife.ui.OptionsDialog</code> to manage the templates available to
 * all instances of <code>RSyntaxTextArea</code>.
 * 
...
public class TemplateOptionPanel extends OptionsDialogPanel {
	...
	public TemplateOptionPanel() {
		...
		msg = ResourceBundle
				.getBundle("org.fife.ui.rsyntaxtextarea.TemplateOptionPanel");
		...
		contentPane
				.setBorder(new OptionPanelBorder(msg.getString("Templates")));
		add(contentPane);
	...
	/**
	 * Returns the <code>JComponent</code> at the "top" of this Options panel.
	 * This is the component that will receive focus if the user switches to
	 * this Options panel in the Options dialog. As an added bonus, if this
	 * component is a <code>JTextComponent</code>, its text is selected for easy
	 * changing.
	 */
			...
			for (int i = 0; i < count; i++) {
				tableModel
						.addRow(new Object[] {
								...
								// Deep copy.
								new TemplateWrapper((CodeTemplate) templates[i]
										.clone()), });
			}
		}
	}
	...
	/**
	 * Updates <code>RSyntaxTextArea</code>'s code template manager with the
	 * information entered here by the user.
	 */
	private void updateCodeTemplateManager() {
		...
		if (ctm != null) {
			...
			for (int i = 0; i < count; i++) {
				...
				TemplateWrapper w = (TemplateWrapper) tableModel.getValueAt(i,
						1);
	...
	class TemplateDialog extends EscapableDialog implements ActionListener,
		...
		public TemplateDialog() {
			...
			if (doc instanceof AbstractDocument) {
				((AbstractDocument) doc)
						.setDocumentFilter(new TemplateNameDocumentFilter());
			}
		...
		/**
		 * @return The after-caret text entered, or <code>null</code> if the
		...
		/**
		 * @return The before-caret text entered, or <code>null</code> if the
		 *         dialog was cancelled.
		 */
		...
		/**
		 * @return The id entered, or <code>null</code> if the dialog was
		 *         cancelled.
		 */
		public char[] getID() {
			return id;
	...
	/**
	 * A document filter that only allows letters, numbers, and '_' to go
	 * through.
	 */
	private static class TemplateNameDocumentFilter extends PickyDocumentFilter {
		...
		protected String cleanseImpl(String text) {
			...
			for (int i = 0; i < length; i++) {
				if (!RSyntaxUtilities.isLetterOrDigit(text.charAt(i))
						&& text.charAt(i) != '_') {
	 ...
	 * SubstanceLookAndFeel makes it impossible to subclass Default*Renderer and
	 * still look "right," so we instead render wrapper classes.
	 */
