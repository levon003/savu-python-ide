...
public class RTextPreferences extends GUIApplicationPreferences
	 ...
	 */
	public static GUIApplicationPreferences generatePreferences(Savu rtext) {
		...
		props.dividerLocations[Savu.TOP]	= rtext.getSplitPaneDividerLocation(Savu.TOP);
		props.dividerLocations[Savu.LEFT] = rtext.getSplitPaneDividerLocation(Savu.LEFT);
		props.dividerLocations[Savu.BOTTOM] = rtext.getSplitPaneDividerLocation(Savu.BOTTOM);
		props.dividerLocations[Savu.RIGHT]= rtext.getSplitPaneDividerLocation(Savu.RIGHT);
		...
		props.tutorialEnabled = true;

		// Save the actions.
		props.accelerators = new HashMap();
		for (int i=0; i<Savu.actionNames.length; i++) {
			String actionName = Savu.actionNames[i];
	...
	public static String getLookAndFeelToLoad() {
		Preferences prefs = Preferences.userNodeForPackage(Savu.class);
	...
	public static GUIApplicationPreferences loadPreferences() {
		...
		try {
			...
			//prefs.clear(); //TODO There's gotta be a better way to reset the default preferences. Also it screwed up all the plugins for some reason?
			...
			props.dividerLocations[Savu.TOP]	= prefs.getInt("pluginDividerLocation.top", props.dividerLocations[Savu.TOP]);
			props.dividerLocations[Savu.LEFT] = prefs.getInt("pluginDividerLocation.left", props.dividerLocations[Savu.LEFT]);
			props.dividerLocations[Savu.BOTTOM] = prefs.getInt("pluginDividerLocation.bottom", props.dividerLocations[Savu.BOTTOM]);
			props.dividerLocations[Savu.RIGHT]= prefs.getInt("pluginDividerLocation.right", props.dividerLocations[Savu.RIGHT]);
			...
			//Added by PyDE
			props.tutorialEnabled = prefs.getBoolean("tutorialEnabled", props.tutorialEnabled);
	...
	public void savePreferences(Object rtext) {
		...
		prefs.putInt("pluginDividerLocation.top",		dividerLocations[Savu.TOP]);
		prefs.putInt("pluginDividerLocation.left",		dividerLocations[Savu.LEFT]);
		prefs.putInt("pluginDividerLocation.bottom",		dividerLocations[Savu.BOTTOM]);
		prefs.putInt("pluginDividerLocation.right",		dividerLocations[Savu.RIGHT]);
		...
		if (backgroundObject instanceof Color) {
			...
			prefs.put("backgroundObject",				"image," + ((Savu)rtext).getMainView().getBackgroundImageFileName());
		}
		...
		prefs.putBoolean("tutorialEnabled", 		tutorialEnabled);
