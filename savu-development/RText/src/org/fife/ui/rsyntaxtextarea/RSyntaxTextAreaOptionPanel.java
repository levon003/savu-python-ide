...
import org.fife.rtext.SavuSyntaxScheme;
import org.fife.ui.*;
 ...
 * @version 0.5
 */
public class RSyntaxTextAreaOptionPanel extends OptionsDialogPanel implements
	...
	//private static final String[] SAMPLES = { "previewJava.txt",
	//		"previewPerl.txt", "previewXml.txt", };
	private static final String SAMPLE = "previewPython.txt";

	/*private static final String[] SAMPLE_STYLES = {
			SyntaxConstants.SYNTAX_STYLE_JAVA,
			SyntaxConstants.SYNTAX_STYLE_PERL,
			SyntaxConstants.SYNTAX_STYLE_XML, };*/
	private static final String SAMPLE_STYLE = SyntaxConstants.SYNTAX_STYLE_PYTHON;
	...
	public RSyntaxTextAreaOptionPanel() {
		...
		ResourceBundle msg = ResourceBundle
				.getBundle("org.fife.ui.rsyntaxtextarea.OptionPanel");
		...
		mainFontSelector.addPropertyChangeListener(FontSelector.FONT_PROPERTY,
				this);
		mainFontSelector.addPropertyChangeListener(
				FontSelector.FONT_COLOR_PROPERTY, this);
		...
		*/
		...
		UIUtil.makeSpringCompactGrid(springPanel, 1, 2, 0, 0, 4, 4);
		...
		syntaxPanel.setBorder(new OptionPanelBorder(msg
				.getString("FontsAndColors")));
		...
		syntaxListModel
				.addElement(msg.getString("Style.Comment.Documentation"));
		...
		syntaxListModel.addElement("<html><b>"
				+ msg.getString("Style.Identifier.PlainText"));
		...
		syntaxListModel.addElement(msg
				.getString("Style.MarkupTag.AttributeValue"));
		...
		syntaxListModel.addElement(msg
				.getString("Style.MarkupTag.ProcessingInstruction"));
		syntaxListModel.addElement(msg
				.getString("Style.MarkupTag.CDataDelimiter"));
		syntaxListModel.addElement(msg.getString("Style.MarkupTag.CData"));
		syntaxListModel.addElement(msg
				.getString("Style.MarkupTag.EntityReference"));
		...
		fontSelector
				.addPropertyChangeListener(FontSelector.FONT_PROPERTY, this);
		fontSelector.addPropertyChangeListener(FontSelector.ENABLED_PROPERTY,
				this);
				*/
		...
		//final String[] samples = { "Java", "Perl", "XML", }; //CHANGE
		//final String sample = "Python";
	...
	public void actionPerformed(ActionEvent e) {
		...
		// If the user clicked the "foreground" check box.
			...
			colorScheme.getStyle(indexToStyle(i)).foreground = selected ? foregroundButton
					.getColor() : null;
			refreshSyntaxHighlightingSection();
			hasUnsavedChanges = true;
		...
		// If the user clicked the "background" check box.
			...
			// Update our color scheme.
			int i = syntaxList.getSelectedIndex();
			colorScheme.getStyle(indexToStyle(i)).background = selected ? backgroundButton
					.getColor() : null;
			refreshSyntaxHighlightingSection();
			hasUnsavedChanges = true;
			firePropertyChange(SYNTAX_COLOR_PROPERTY, null, null);
...
*/
		...
		if ("RestoreDefaults".equals(command)) {
			...
			//PyDE
			SavuSyntaxScheme defaultScheme = new SavuSyntaxScheme(true);
			...
			if (!getTextAreaFont().equals(defaultFont)
				...
				//COLORCHANGE
				setBackgroundObject(new Color(50,50,40));
	...
	protected void doApplyImpl(Frame owner) {
		...
		rtext.setSyntaxScheme(getSyntaxScheme()); // Doesn't update if it
													// doesn't have to.
	}
	...
	/**
	 * Returns the name of the file containing the chosen background image. If
	 * the user selected a color for the background, this method returns
	 * <code>null</code>.
	 * 
	 ...
	 * 
	 * @param underline
	 *            Whether the preview text should be underlined.
	 ...
	 * <code>SyntaxScheme</code>. This is necessary because the list does not
	 * display an item for <code>Style</code> index 0 (token type
	 * <code>NULL</code>).
	 * 
	 * @param index
	 *            An index into the <code>JList</code>.
...
*/
			...
			refreshSyntaxHighlightingSection();
			hasUnsavedChanges = true;
			firePropertyChange(UNKNOWN_PROPERTY, e.getOldValue(),
					e.getNewValue());
		}
	...
	private void refreshDisplayedSample() {
		...
		InputStream in = getClass().getResourceAsStream(SAMPLE);
		...
		//sampleArea.setSyntaxEditingStyle(SAMPLE_STYLES[index]); CHANGE
		sampleArea.setSyntaxEditingStyle(SAMPLE_STYLE);
	...
	/*//*
	 * Sets the name of the file containing the background image. If the initial
	 * background object is a color, you should pass <code>null</code> to this
	 * method.
	 * 
	 * @param name
	...
	private void setBackgroundObject(Object background) {
		...
		if (sampleBG instanceof Image) { // Lighten as RText will
			Savu parent = (Savu) getOptionsDialog().getParent();
			sampleBG = RTextUtilities.getTranslucentImage(parent,
					(Image) sampleBG, parent.getMainView()
	 ...
	 * 
	 * @param ss
	 *            The syntax highlighting color scheme to display.
	 * @see #getSyntaxScheme
	 */
	...
	protected void setValuesImpl(Frame owner) {
		...
		setTextAreaFont(mainView.getTextAreaFont(),
				mainView.getTextAreaUnderline());
	...
	*/
	/*public void updateUI() {
		super.updateUI();
		if (backgroundDialog != null) {
			SwingUtilities.updateComponentTreeUI(backgroundDialog); // Updates
																	// dialog.
