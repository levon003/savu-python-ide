...
/**
 * Action used by an <code>AbstractMainView</code> to open a document in a new
 * window.
 * 
...
class OpenInNewWindowAction extends StandardAction {
	...
	private Savu newWindow; // Only run on EDT so it's okay to cache this.
	 ...
	 */
	public OpenInNewWindowAction(Savu owner, ResourceBundle msg, Icon icon) {
	...
	public synchronized void actionPerformed(ActionEvent e) {
		...
		newWindow = new Savu(null,
				(RTextPreferences) RTextPreferences
						.generatePreferences((Savu) getApplication()));
	...
	/**
	 * Opens the new RText's file chooser for the user to select a file(s) to
	 * open.
	 * 
	 * @author Robert Futrell
	 * @version 1.0
	...
	private class OpenFileChooserRunnable implements Runnable {
		...
		public void run() {
			...
			// chooser.setUnderlinedFiles(null); // Unnecessary since no files
			// are yet open!
			...
			if (returnVal == javax.swing.JFileChooser.APPROVE_OPTION) {
				...
				RTextEditorPane newCurrentTextArea = newMainView
						.getCurrentTextArea();
				...
				if (newMainView.getNumDocuments() == 1
						&& newCurrentTextArea.getFileName().equals(
								owner.getNewFileName())
