...
public abstract class AbstractGUIApplication extends JFrame
	...
	//Added by PyDE
	/**
	 * A panel set invisible by default that will contain content used by the tutorial.
	 */
	...
	/**
	 * Returns the tutorial panel this application is using.
	...
	private void possibleMacOSXRegistration() {
		...
		if (getOS()==OS_MAC_OSX) {
			...
			try {
				...
				//TODO LOW PRIO - Load this resource from a more specialized location
				Image icon = ImageIO.read(new File("lib/app_icon.png"));
	...
	private class StartupRunnable implements Runnable {
		...
		public void run() {
			...
			toolBarPanels[0].add(newContentPane);
			newContentPane.add(mainContentPanel, BorderLayout.CENTER);
			...
			newContentPane.add(tutorialPanel, BorderLayout.NORTH);
