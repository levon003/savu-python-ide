 ...
 * 12/17/2010
 *
 * RunTextArea.java - Text component for the run.
...
/**
 * Text component that displays output.
 * 
 * @author Robert Futrelle Jr.
 * @version 1.0
 */
abstract class RunTextArea extends JTextPane {

	//COLORCHANGE - Run Output text colors
	public static final Color DEFAULT_STDOUT_FG = new Color(255,240,210);
	
	public static final Color DEFAULT_STDIN_FG = new Color(120,200,110);

	public static final Color DEFAULT_STDERR_FG = new Color(240,90,20);
	
	public static final Color DEFAULT_ERRORLINK_FG = new Color(60,150,230);
	...
	public static final String STYLE_ERRORLINK = "errorlink";

	protected RunPlugin plugin;
	...
	/**
	 * The maximum number of lines to display in the run.
	 */
	 ...
	 * Constructor.
	 */
	public RunTextArea(RunPlugin plugin) {
		this.plugin = plugin;
		this.errorLinker = plugin.getErrorLinkController();
		//COLORCHANGE
		setBackground(Savu.OUTPUT_AREA_BACKGROUND_COLOR); 
	 ...
	 *            The text to append.
	 * @param style
	 *            The style to apply to the appended text.
	 */
	 ...
	 * @param style
	 *            The style to apply to the appended text.
	 * @param treatAsUserInput
	 *            Whether to treat the text as user input. This determine
	 *            whether the user can use backspace to remove this text or not.
	 */
	protected void appendImpl(final String text, final String style,
		...
		if (SwingUtilities.isEventDispatchThread()) {
			...
			try {
				...
				if (style == STYLE_STDERR) {
					errorLinker.reportErrorOutput(text, end, doc.getLength(), this);
				}
			...
			// Don't let the run's text get too long
	...
	/**
	 * Clears this run. This should only be called on the EDT.
	 */
	public void clear() {
		Document doc = getDocument();
		setSelectionStart(0);
	...
	/**
	 * Allows constructors to do stuff before the initial {@link #clear()} call
	 * is made. The default implementation does nothing.
	 */
	protected void init() {
	}

	/**
	 * Installs the styles used by this text component. Uses RSyntaxTextArea's current text area size as the base font, or the default font if no current textarea exists.
	 * 
	 * @param checkForSubstance
	 *            Whether to work around a Substance oddity (Insubstantial
	 *            7.2.1).
	 */
	private void installDefaultStyles(boolean checkForSubstance) {
		Font font = null;
		try {
			font = this.plugin.getRText().getMainView().getCurrentTextArea().getFont();
		} catch (Exception ex) {
			font = RTextArea.getDefaultFont();
		}
		installDefaultStyles(font, checkForSubstance);
	}
	
	/**
	 * Installs the styles used by this text component.
	 * 
	 * @param font The font to use as a base for this component's style.
	 * @param checkForSubstance
	 *            Whether to work around a Substance oddity (Insubstantial
	 *            7.2.1).
	 */
	private void installDefaultStyles(Font font, boolean checkForSubstance) {
		...
		StyleConstants.setForeground(stdin, DEFAULT_STDIN_FG);
		...
		Style errorlink = addStyle(STYLE_ERRORLINK, defaultStyle);
		StyleConstants.setForeground(errorlink, DEFAULT_ERRORLINK_FG);
		StyleConstants.setUnderline(errorlink, true);
	...
	/**
	 * Replaces the command entered thus far with another one. This is used when
	 * the user cycles through the command history. This method should only be
	 * called on the EDT.
	 * 
	 * @param command
	 *            The command to replace with.
	 */
	 ...
	 * 
	 * @param tabSize
	 *            The new tab size, in characters.
	 */
	 ...
	 * 
	 * @param e
	 *            The location at which to display the popup.
	 */
	...
	private class DeletePreviousWordAction extends TextAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				end = Utilities.getPreviousWord(RunTextArea.this, start);
	...
	private class SubmitAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			RunTextArea.super.replaceSelection("\n");
			...
			if (plugin.getCurrentProcess() != null && plugin.getCurrentProcess().isRunning())
			{
				plugin.getCurrentProcess().writeInput(text+"\n");
			}
