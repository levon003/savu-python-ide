...
/**
 * Options panel containing only code folding options. Useful for languages that
 * only provide folding support but not code completion.
 * 
...
class FoldingOnlyOptionsPanel extends OptionsDialogPanel {
	 ...
	 * Constructor.
	 */
	public FoldingOnlyOptionsPanel(String nameKey, String icon, String language) {
	...
	protected void doApplyCodeFoldingPreference(Savu rtext) {
		AbstractMainView view = rtext.getMainView();
		view.setCodeFoldingEnabledFor(language, enabledCB.isSelected());
	}

	/**
	 * Applies the selected code folding preference. Subclasses can override if
	 * they display more options.
	 * 
	 * @param owner
	 *            The parent {@link Savu} application.
	 */
	protected final void doApplyImpl(Frame owner) {
		doApplyCodeFoldingPreference((Savu) owner);
	}
	...
	protected void setCodeFoldingValueImpl(Savu rtext) {
		AbstractMainView view = rtext.getMainView();
	...
	protected final void setValuesImpl(Frame owner) {
		setCodeFoldingValueImpl((Savu) owner);
	}
