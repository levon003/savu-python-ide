...
public class CustomizableToolBar extends JToolBar {
	...
	protected void populateAddRemovePopupMenu(ResourceBundle msg) {
		...
		for (int i=0; i<num; i++) {
			if (components[i] instanceof JButton) {
				...
				final JComboBox<String> comboBox = (JComboBox<String>) components[i];
				String title = comboBox.getToolTipText();
				if (title==null)
					title = msg.getString("PopupMenu.Unknown.txt");
				cbMenuItem = new JCheckBoxMenuItem(
					new AbstractAction(title) {
				...
				cbMenuItem.setSelected(comboBox.isVisible());
				popupMenu.add(cbMenuItem);
			}
		}
		popupMenu.addSeparator();
	...
	private class ResetAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			//Need to recreate the popup menu, as menu item visibilities may have changed unreported
			createPopupMenu();
		}
