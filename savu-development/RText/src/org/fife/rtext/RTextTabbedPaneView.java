...
public class RTextTabbedPaneView extends AbstractMainView implements ChangeListener {
	 ...
	 *        this tabbed pane.
	 */
	public RTextTabbedPaneView(Savu owner, String[] filesToOpen,
							RTextPreferences properties) {

		setLayout(new GridLayout(1,1));
		tabbedPane = new TabbedPane();
		tabbedPane.addChangeListener(this);
//COLORCHANGE - place to change colors of filenames
		add(tabbedPane);
	...
	protected void addTextAreaImpl(String title, Component component,
		...
		URL url7 = getClass().getResource("close.gif");
		try {
			closeIcon = new ImageIcon(ImageIO.read(url7));
		}
		catch (IOException ioe){

		}
		tabbedPane.addTab(title,  temp);
...
//		}
	...
	public void setDocumentDisplayNameAt(int index, String displayName) {
		...
		tabbedPane.setTabComponentAt(index, setTitlePanel((JPanel)tabbedPane.getComponentAt(index), displayName, closeIcon, getIconFor(getRTextScrollPaneAt(index)))); 
	}
	...
	/**
	 * Properly formats the titlePanel of tabs in tabbedPane to have a close button.
	 * @param panel Panel to change the titlepanel for
	 * @param title Name of the panel to put as the title
	 * @param icon Close button icon
	 * @return
	 */
	private JPanel setTitlePanel(final JPanel panel, String title, Icon icon, Icon icon2) { //TODO test
		...
		closeButton.setBackground(Savu.OUTPUT_AREA_BACKGROUND_COLOR);
		...
		closeButton.addMouseListener(new MouseAdapter()
		{
			...
			public void mouseClicked(MouseEvent e)
			{
				if (SwingUtilities.isLeftMouseButton(e) && !e.isPopupTrigger()) {
					...
					int index = tabbedPane.indexAtLocation((int) p.getX(), (int) p.getY());
					System.out.println(index);
					if (index!=-1) {
						Component oldSelection = tabbedPane.getSelectedComponent();
						RTextTabbedPaneView.this.setSelectedIndex(index);
						RTextTabbedPaneView.this.closeCurrentDocument();
						int oldComponentIndex = tabbedPane.indexOfComponent(oldSelection);
						if (oldComponentIndex != index) { //We didn't try to close the currently selected frame
							RTextTabbedPaneView.this.setSelectedIndex(oldComponentIndex); //Return to the current document
						}
					}
				}
			}
	...
	public class TabbedPane extends JTabbedPane
		...
		public TabbedPane() {
            ...
            this.setUI(new HiFiTabbedPaneUI());
		...
		public void updateNames() {
				RTextTabbedPaneView tpv = (RTextTabbedPaneView) getParent();
				for (int i = 0; i < tpv.getNumDocuments(); ++i) {
					tpv.setDocumentDisplayNameAt(i, tpv.getDocumentDisplayNameAt(i));
		...
		protected JPopupMenu getTabPopupMenu() {
			if (popup==null) {
				...
				popup.add(item);
				item = new JMenuItem(owner.getAction(Savu.CLOSE_ALL_ACTION));
