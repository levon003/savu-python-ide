...
class AutoCompletePopupWindow extends JWindow implements CaretListener,
	...
	private int maxNumberOfCompletions;
	...
	public AutoCompletePopupWindow(Window parent, final AutoCompletion ac) {
		...
		maxNumberOfCompletions = 11;
		this.ac = ac;
		model = new CompletionListModel();
		list = new PopupList(model);
		//COLORCHANGE AutoComplete popup background and text
		list.setBackground(new Color(80,80,70));
		list.setForeground(new Color(125,210,250));
		...
		//COLORCHANGE popup window border
		sp.getVerticalScrollBar().getParent().setBackground(new Color(80,80,70));
		AbstractLookAndFeel.setTheme(new HiFiDefaultTheme());
		sp.getVerticalScrollBar().setUI(new SavuScrollbarUI());
		sp.getVerticalScrollBar().setBackground(new Color(40,40,35)); //TODO set this properly
		sp.getHorizontalScrollBar().setUI(new SavuScrollbarUI());
		...
		// here.
		JPanel corner = new org.fife.ui.SizeGrip();
	...
	/**
	 * Returns the current max number of completions to be displayed at once in the window
	 */
	public int getMaxNumberOfCompletions(){
		return maxNumberOfCompletions;
	}
	...
	/**
	 * Sets the maximum number of completions that should be visible in the window at one time
	 * 
	 * @param num
	 */
	public void setMaxNumberOfCompletions(int num){
		maxNumberOfCompletions = num;
	}
	/**
