...
/**
 * Base class for tree nodes in an <code>DefaultSourceTree</code>. They can be
 * sorted and filtered based on user input.
 * <p>
 * This class is a clone of the identical class in
 * <code>RSTALanguageSupport</code>, but unfortunately exists to prevent a
 * dependency on that library in this plugin.
 * 
...
public class SourceTreeNode extends DefaultMutableTreeNode {
	...
	public void add(MutableTreeNode child) {
		...
		if (sortable && sorted) {
			refreshVisibleChildren(); // TODO: Find index and add for
										// performance
		}
	}
	...
	/**
	 * Returns a comparator used to sort the child nodes of this node. The
	 * default implementation sorts alphabetically. Subclasses may want to
	 * override to return a comparator that groups by type of node and sorts by
	 * group, etc.
	 * 
	 * @return A comparator.
	 */
				...
				for (Iterator i = visibleChildren.iterator(); i.hasNext();) {
					...
					if (node.isLeaf()) {
						...
						if (!text.toLowerCase().startsWith(prefix)) {
							// System.out.println("Removing tree node: " +
							// text);
							i.remove();
						}
					}
	 ...
	 * 
	 * @param sortable
	 *            Whether this node's children are sortable.
	 * @see #isSortable()
	 */
