...
public class ToolPlugin extends GUIPlugin implements PropertyChangeListener {
	 ...
	 * called on the EDT.
	 * 
	 * @return The currently running tool, or <code>null</code> if a tool isn't
	 *         running.
	 */
	...
	/**
	 * Loads saved preferences for this plugin. If this is the first time
	 * through, default values will be returned.
	 * 
	...
	/**
	 * Refreshes the elements in the Tools menu to be in sync with the tools the
	 * user has defined.
	 */
	private void refreshToolMenu() {
		...
		if (ToolManager.get().getToolCount() > 0) {
			for (Iterator i = ToolManager.get().getToolIterator(); i.hasNext();) {
				...
				RunToolAction a = new RunToolAction(app, tool,
						getDockableWindow());
				toolsMenu.add(createMenuItem(a));
			}
		} else {
