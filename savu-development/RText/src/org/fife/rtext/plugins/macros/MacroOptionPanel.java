 ...
 * @version 1.0
 */
class MacroOptionPanel extends PluginOptionsDialogPanel implements
	...
	public MacroOptionPanel(MacroPlugin plugin) {
		...
		cp.setBorder(new OptionPanelBorder(plugin
				.getString("Options.Section.MacroList")));
		add(cp);
		...
		macroTable = new ModifiableTable(model, ModifiableTable.BOTTOM,
				ModifiableTable.ADD_REMOVE_MODIFY, customButtons);
	 ...
	 * 
	 * @param macro
	 *            The macro to add to the table.
	 */
	private void addTableRowForMacro(Macro macro) {
		model.addRow(new Object[] { macro.clone(),
				KeyStroke.getKeyStroke(macro.getAccelerator()), macro.getDesc() });
	}
	...
	private File getExampleMacrosDir() {
		Savu app = ((MacroPlugin) getPlugin()).getRText();
		String installDir = app.getInstallLocation();
		return new File(installDir, "exampleMacros");
	}

	/**
	 * Returns whether the table in this option panel contains a macro with a
	 * given name.
	 * 
	 * @param name
			...
			if (dot > -1) {
				...
				if ("js".equalsIgnoreCase(extension)
					...
					if (getTableContainsMacroNamed(macroName)) {
						int count = 1;
						while (getTableContainsMacroNamed(macroName + "_"
								+ count)) {
							count++;
						}
						macroName += "_" + count;
