...
public class FolderFilterInfo {
	 ...
	 * @param isDir
	 *            Whether the file is a directory (passed in for performance).
	 ...
	 * @param matchers
	 *            The array of matchers to check against.
	 * @return Whether the file name matched any of the matchers.
	 */
	private static final boolean matches(String fileName, NameMatcher[] matchers) {
	...
	/**
	 * Converts an array of wildcard file filters into an array of matchers for
	 * them.
	 * 
	 * @param filters
	 *            The file filters.
		...
		if (filters != null && filters.length > 0) {
			...
			for (int i = 0; i < filters.length; i++) {
				if (isStringLiteral(filters[i])) {
					...
					Pattern pattern = RTextUtilities.getPatternForFileFilter(
							filters[i], true);
	...
	private static class StringLiteralNameMatcher implements NameMatcher {
		...
		public boolean matches(String text) {
			return RTextUtilities.isFileSystemCaseSensitive() ? literal
					.equals(text) : literal.equalsIgnoreCase(text);
		}
