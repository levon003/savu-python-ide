...
/**
 * A window that displays text flagged in source code comments that have been
 * designated as "tasks." Tasks are identified by configurable identifiers, such
 * as "<code>FIXME</code>", "<code>TODO</code>" and "<code>HACK</code>".
 * <p>
 * 
 ...
 * @version 1.0
 */
class TaskWindow extends AbstractParserNoticeWindow implements
	...
	public TaskWindow(Savu rtext, String taskIdentifiers) {
	 ...
	 * 
	 * @param textArea
	 *            The text area to start parsing for tasks.
	 ...
	 *         parsing is disabled. If there are no identifiers, an empty string
	 *         is returned.
	...
	private void installParser() {
		if (!installed) {
			...
			mainView.addPropertyChangeListener(
					AbstractMainView.TEXT_AREA_ADDED_PROPERTY, this);
			mainView.addPropertyChangeListener(
					AbstractMainView.TEXT_AREA_REMOVED_PROPERTY, this);
	 ...
	 * 
	 * @param textArea
	 *            The text area to stop parsing for tasks.
	 ...
	 * 
	 * @param active
	 *            Whether the task window should be active.
	 */
	 ...
	 * 
	 * @param identifiers
	 *            The identifiers, separated by the '<code>|</code>' character.
	 * @return Whether the task pattern was actually modified. This will be
	 *         <code>false</code> if <code>identifiers</code> is the same as the
	 *         current value.
	...
	private void uninstallParser() {
		if (installed) {
			Savu rtext = getRText();
			AbstractMainView mainView = rtext.getMainView();
			mainView.removePropertyChangeListener(
					AbstractMainView.TEXT_AREA_ADDED_PROPERTY, this);
			mainView.removePropertyChangeListener(
					AbstractMainView.TEXT_AREA_REMOVED_PROPERTY, this);
			for (int i = 0; i < mainView.getNumDocuments(); i++) {
			...
			for (Iterator i = notices.iterator(); i.hasNext();) {
				...
				if (notice.getParser() == taskParser) {
					Object[] data = { getIcon(), textArea,
							// Integer.intValue(notice.getValue()+1) // TODO:
							// 1.5
