...
/**
 * Pretty printer for HTML and XML. Currently simply delegates to <a
...
class MarkupPrettyPrinter implements PrettyPrinter, TidyMessageListener {
	...
	MarkupPrettyPrinter(Savu rtext, Plugin plugin) {
		this.rtext = rtext;
		this.plugin = plugin;
	}
	...
	public PrettyPrintResult prettyPrint(String text) {
		...
		if (xml) {
			...
			tidy.setMakeClean(opts.getMakeClean()); // Replace presentational
													// tags
			spaces = opts.getSpaceCount();
			tidy.setSpaces(spaces < 0 ? 1 : spaces); // Convert to tabs later
