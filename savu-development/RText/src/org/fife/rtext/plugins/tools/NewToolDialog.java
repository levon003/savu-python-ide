...
public class NewToolDialog extends EscapableDialog implements ActionListener {
	...
	/**
	 * The original name of the tool being edited, or <code>null</code> if the
	 * user is creating a new tool.
	 */
	 ...
	 * @param key
	 *            The key for the localized tool variable.
	 */
	 ...
	 * 
	 * @return The tool, or <code>null</code> if there was a problem with their
	 *         input. In the latter case, a notice has been displayed to the
	...
	private void createGUI() {
		...
		JPanel nameFieldPanel = RTextUtilities.createAssistancePanel(nameField,
				null);
		JLabel descLabel = new JLabel(msg.getString("Description"));
		descField = new JTextField(40);
		JPanel descFieldPanel = RTextUtilities.createAssistancePanel(descField,
				null);
		JLabel programLabel = new JLabel(msg.getString("Program"));
		programField = new FSATextField();
		JPanel programFieldPanel = RTextUtilities.createAssistancePanel(
				programField, null);
		...
		provider.addCompletion(new BasicCompletion(provider, "${file_dir}",
		...
		JPanel dirFieldPanel = RTextUtilities.createAssistancePanel(dirField,
				aip);
		JButton dirBrowseButton = UIUtil.newTabbedPaneButton(msg
				.getString("Browse"));
		...
		JPanel shortcutFieldPanel = RTextUtilities.createAssistancePanel(
				shortcutField, null);

		Dimension dim = new Dimension(1, 1); // MUST have finite width!
		if (o.isLeftToRight()) {
			springPanel.add(nameLabel);
			springPanel.add(nameFieldPanel);
			springPanel.add(Box.createRigidArea(dim));
			springPanel.add(descLabel);
			springPanel.add(descFieldPanel);
			springPanel.add(Box.createRigidArea(dim));
			springPanel.add(programLabel);
			springPanel.add(programFieldPanel);
			springPanel.add(programBrowseButton);
			springPanel.add(dirLabel);
			springPanel.add(dirFieldPanel);
			springPanel.add(dirBrowseButton);
			springPanel.add(shortcutLabel);
			springPanel.add(shortcutFieldPanel);
			springPanel.add(Box.createRigidArea(dim));
		} else {
			springPanel.add(Box.createRigidArea(dim));
			springPanel.add(nameFieldPanel);
		...
		temp2.setBorder(BorderFactory.createTitledBorder(msg
				.getString("CommandLineArgs")));
		temp2.add(argTable);
		temp.add(temp2);
		...
		JButton cancelButton = UIUtil.newTabbedPaneButton(msg
				.getString("Cancel"));
		cancelButton.setActionCommand("Cancel");
		cancelButton.addActionListener(this);
		Container buttonPanel = UIUtil.createButtonFooter(okButton,
				cancelButton);
	 ...
	 * 
	 * @return The tool, or <code>null</code> if the user canceled the dialog.
	 * @see #setTool(Tool)
	 */
	public Tool getTool() {
		return tool;
	...
	public void setTool(Tool tool) {
		...
		shortcutField
				.setKeyStroke(KeyStroke.getKeyStroke(tool.getAccelerator()));
	 ...
	 * 
	 * @param comp
	 *            The component with invalid input.
	 * @param key
	 *            A key into the resource bundle for the error.
	 * @param param
	 *            An optional parameter for the localized error. This may be
	 *            <code>null</code>.
	 */
	...
	/**
	 * The dialog that allows the user to add or modify a command line argument.
	 */
	private class ArgDialog extends EscapableDialog implements ActionListener,
		...
		public ArgDialog(JDialog parent) {
			...
			JLabel argLabel = new JLabel(
					msg.getString("ArgumentDialog.Argument"));
			...
			aip.setAssistanceEnabled(AbstractSearchDialog
					.getContentAssistImage());
			JPanel argFieldPanel = RTextUtilities.createAssistancePanel(
					argField, aip);
			...
			if (icon != null) {
				...
				iconLabel.setBorder(o.isLeftToRight() ? BorderFactory
	 ...
	 */
	private class EnvVarDialog extends EscapableDialog implements
		...
		public EnvVarDialog(JDialog parent) {
			...
			JPanel nameFieldPanel = RTextUtilities.createAssistancePanel(
					nameField, null);
			...
			aip.setAssistanceEnabled(AbstractSearchDialog
					.getContentAssistImage());
			JPanel valueFieldPanel = RTextUtilities.createAssistancePanel(
					valueField, aip);
			...
			if (parent.getComponentOrientation().isLeftToRight()) {
				temp.add(nameLabel);
				temp.add(nameFieldPanel);
				temp.add(Box.createRigidArea(dim));
				temp.add(valueLabel);
				temp.add(valueFieldPanel);
				temp.add(varButton);
			} else {
				temp.add(Box.createRigidArea(dim));
				temp.add(nameFieldPanel);
		...
		public String[] getReturnValue() {
			return escaped ? null : new String[] { nameField.getText(),
					valueField.getText() };
		}
	...
	/**
	 * A button with a popup menu allowing the insertion of variables into a
	 * text field.
	 */
	private static class VariableButton extends MenuButton {
		...
		public VariableButton(JTextField field) {
			...
			addMenuItem(new VariableAction("Variable.FileName", "${file_name}",
					field));
			addMenuItem(new VariableAction("Variable.FileNameNoExt",
					"${file_name_no_ext}", field));
			addMenuItem(new VariableAction("Variable.FileDir", "${file_dir}",
					field));
			addMenuItem(new VariableAction("Variable.FileFullPath",
					"${file_full_path}", field));
		}
