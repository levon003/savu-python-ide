...
public class AutoCompletion {
	 ...
	 */
	private PythonCompletionProvider pyProvider;
	...
	public AutoCompletion(PythonCompletionProvider provider) {
	...
	protected int refreshPopupWindow() {
		...
		String text = provider.getAlreadyEnteredText(textComponent, true);
		...
		if (count > 1 || (count == 1 && (isPopupVisible() || textLen == 0))
			...
			// Varies the size of the window based on how many completions there are
			int completionsInWindow = popupWindow.getMaxNumberOfCompletions();
			if (completions.size()<popupWindow.getMaxNumberOfCompletions()){
				completionsInWindow = completions.size()+1;
			}
			popupWindow.setSize(new Dimension(popupWindow.getSize().width, completionsInWindow * (popupWindow.getFontMetrics(popupWindow.getFont()).getHeight())+6));
	 ...
	 *         <code>null</code>.
	 */
	public void setCompletionProvider(PythonCompletionProvider provider) {
		if (provider == null) {
	...
	/**
	 * Public method to remove the current trigger key
	 * 
	 * @see #uninstallTriggerKey()
	 */
	public void removeTriggerKey(){
		uninstallTriggerKey();
	}
