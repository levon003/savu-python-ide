...
public class SpellingOptionPanel extends OptionsDialogPanel {
	...
	public SpellingOptionPanel() {
		...
		msg = ResourceBundle
				.getBundle("org.fife.ui.rsyntaxtextarea.SpellingOptionPanel");
		...
		userDictDescField = new SelectableLabel(
				msg.getString("UserDictionaryDesc"));
		...
		((AbstractDocument) maxErrorsField.getDocument())
		...
		viewSpellingWindowCB = new JCheckBox(
				msg.getString("ViewSpellingErrorWindow"));
		...
		if (res == null) {
			...
			if (userDict != null) {
				if (userDict.isDirectory()) {
					String desc = msg
							.getString("Error.UserDictionaryIsDirectory.txt");
					res = new OptionsPanelCheckResult(this, userDictField, desc);
				} else {
					File parent = userDict.getParentFile();
					if (parent == null || !parent.exists()) {
						String desc = msg
								.getString("Error.CannotCreateUserDictionary.txt");
						res = new OptionsPanelCheckResult(this, userDictField,
								desc);
					}
				}
			}
	...
	private int getMaxSpellingErrors() {
		try {
			...
			return RTextPreferences.DEFAULT_MAX_SPELLING_ERRORS; // Default
																	// value
		}
	}
	...
	protected void setValuesImpl(Frame owner) {
		...
		maxErrorsField.setText(Integer.toString(support.getMaxSpellingErrors()));
	}
	...
	private class Listener implements DocumentListener, ActionListener,
		...
		public void actionPerformed(ActionEvent e) {
			...
			if ("Enabled".equals(command)) {
				...
				String defaultMaxErrors = Integer
						.toString(RTextPreferences.DEFAULT_MAX_SPELLING_ERRORS);

				if (enabledCB.isSelected() || dictCombo.getSelectedIndex() != 1
